...
module.exports = async (req, res) => {
	...
	try {
		let postSearched = await db.query("SELECT post_id, post_title, user_name, post_thumbnail FROM posts WHERE post_title LIKE $1 OR post_title LIKE UPPER($1) OR post_title LIKE LOWER($1) OR post_title LIKE INITCAP($1) OR post_title LIKE $2 OR post_title LIKE UPPER($2) OR post_title LIKE LOWER($2) OR post_title LIKE INITCAP($2) OR post_title LIKE $3 OR post_title LIKE UPPER($3) OR post_title LIKE LOWER($3) OR post_title LIKE INITCAP($3) OR post_title LIKE $4 OR post_title LIKE UPPER($4) OR post_title LIKE LOWER($4) OR post_title LIKE INITCAP($4) ORDER BY id DESC LIMIT 100", [post_title, post_title + '%', '%' + post_title, '%' + post_title + '%']);
